...
var (
	...
	hasUnsupportedAlgo bool              // Flag to track if we found GOST or unknown algos
)
...
func main() {
	...
	if err != nil {
		if hasUnsupportedAlgo {
			logNormal("\n⚠️  CRITICAL: This certificate uses an algorithm unsupported by Go (e.g. GOST).\n")
...
// downloadCertFile - HARDENED VERSION
func downloadCertFile(urlStr string) []*x509.Certificate {
	...
	// STRICT CHECK: Only allow http and https
	...
	// 2. Strict Client Configuration
	client := &http.Client{
		Timeout: 10 * time.Second,
		// Optional: Prevent excessive redirects
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			if len(via) >= 3 { // Limit max redirects to 3
	...
	// 4. LIMIT FILE SIZE (Critical Anti-DoS)
	// Certificates are usually <10KB. 256KB is a nice safety limit.
	const MaxLimit = 0.25 * 1024 * 1024 // 256KB

	limitedBody := io.LimitReader(resp.Body, MaxLimit)
...
func parseCertsFromData(data []byte, source string) []*x509.Certificate {
	...
	// Try parsing as PEM (ASCII) first
	blockData := data
	for {
		var block *pem.Block
	...
	// If no PEM blocks found, try parsing as raw DER (Binary)
	if len(certs) == 0 {
		cert, err := x509.ParseCertificate(data)
		if err == nil {
			// Found valid binary certificate
			return []*x509.Certificate{cert}
		}
		// If it's not PEM and not valid DER, it's garbage.
		exitErr(fmt.Errorf("no certificates found in %s (checked PEM and DER formats)", source))
	}
	return certs
}
...
func fetchAIA(cert *x509.Certificate) (*x509.Certificate, error) {
	...
	for i, url := range cert.IssuingCertificateURL {
		...
		// Use the safe parser to handle both formats from AIA
...
func highlightLeafIssues(cert *x509.Certificate) {
	...
	// Check for Unknown / GOST Algorithms
	if cert.SignatureAlgorithm == x509.UnknownSignatureAlgorithm {
		hasUnsupportedAlgo = true
		logNormal("⚠️  WARNING: Signature Algorithm is UNKNOWN/UNSUPPORTED.\n")
		logNormal("   This usually indicates GOST (Russian), proprietary, or very new algorithms.\n")
	}
